    ...
    public function __construct($rawData, $rules)
    {
        ...
        foreach ($rules as $field => $rule) {
            ...
            $value = isset($normalizedInput[$fieldLower])
                ? trim($normalizedInput[$fieldLower])
                : null;
                ...
                switch ($rule['type']) {
                    ...
                    case 'date':
                        ...
                        if (!($d && $d->format('Y-m-d') === $value)) {
                            $this->errors[$field] =
                                'La fecha no tiene un formato válido (AAAA-MM-DD).';
                        }
                        break;
                    ...
                    case 'int':
                        if (!is_numeric($value)) {
                            ...
                            if (isset($rule['min']) && $value < $rule['min']) {
                                $this->errors[$field] = "Valor mínimo: {$rule['min']}";
                            }
                            if (isset($rule['max']) && $value > $rule['max']) {
                                $this->errors[$field] = "Valor máximo: {$rule['max']}";
                            }
                        }
                        break;
                    ...
                    case 'tel':
                        if (isset($rule['regex']) && !preg_match($rule['regex'], $value)) {
                            $this->errors[$field] =
                                $rule['error'] ?? 'Formato de texto no permitido.';
                        }
                        break;
                }
            }
