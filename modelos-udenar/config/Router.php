...
class Router {
    ...
    public function resolve() {
        ...
        if (!$realTable) return "<div class='alert alert-danger'>Módulo no registrado.</div>";

        $controllerClass = ucfirst($this->tableName) . 'Controller';
        $controller = class_exists($controllerClass) ? new $controllerClass($this->conn) : new GenericController($this->conn, $realTable);
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($this->action === 'create' || $this->action === 'update')) {
